...
class MixedAgent(CaptureAgent):
    ...
    QLWeights = {
        ...
        # {
        #         'offensiveWeights': {
        #             'closest-food': -65.85679140710175,
        #             'bias': 31.24606137091568,
        #             '#-of-ghosts-1-step-away': -76.4178568572925805,
        #             'successorScore': 61.424651333478746,
        #             'stop': -40.593765970997792,
        #             'chance-return-food': 40.170345504248534,
        #             'depositFood': 126.39781746879254,
        #             'eats-ghost': 2,
        #             'enemyscared': 25.119061784976804,
        #             'distance-to-enemyscared': -30.948826137895832,
        #             'closest-capsule': -10.48896366166399
        #         },
        #         'defensiveWeights': {
        #             'onDefense': -1000.4004643698231,
        #             'distance_to_middlle': -265.90128485877499,
        #             'numInvaders': -63.24992549205048,
        #             'invaderDistance': -13.48831877508746,
        #             'stop': -26.759194781371242,
        #             'reverse': -10.345853063149853,
        #             'distaToInvader_noisyDist': -250.79684891226397,
        #             'chaseInvader': -28.249017992554915
        #         },
        #         'escapeWeights': {
        #             'onDefense': 44.202793955836455,
        #             'enemyDistance': 30,
        #             'stop': -100,
        #             'carrying': 10.0,
        #             'distanceToHome': 24.983916859531192
        #         }
        #     }
    }
    ...
    def getOffensiveReward(self, gameState: GameState, nextState: GameState):
        ...
        # nextCapsules = self.getCapsules(nextState)
        # if len(currentCapsules) < len(nextCapsules):
        #     base_reward += 5
        ...
        # if new_food_returned > 0:
        #     # return home with food get reward score
        #     base_reward += new_food_returned * 5
        # # gameState.getLegalActions(self.index)
        # # Calculate distances to the ghosts in the current and next states
        # if len(ghosts) > 0:
        #     currentMinDistToGhost = min([self.getMazeDistance(currentAgentState.getPosition(), g) for g in ghosts])
        ...
        # if the agent die
        # if nextState.getAgentPosition(self.index) == self.startPosition:
        #     base_reward -= 30
        ...
        # if currentDistToFood > nextDistTofood:
        #     base_reward += 12 + 3 / (1+ nextDistTofood)
        # else:
        #     base_reward -= 5
    ...
    def getDefensiveReward(self,gameState: GameState, nextState: GameState):
        ...
        # middle_row = defensiveHalfGrid.height // 2
        # rightmost_point = max((x, y) for x in range(defensiveHalfGrid.width) for y in range(defensiveHalfGrid.height) if defensiveHalfGrid[x][y] and y == middle_row)
    ...
    #     currentAgentState:AgentState = gameState.getAgentState(self.index)
    #     nextAgentState:AgentState = nextState.getAgentState(self.index)
    #     enemy = [gameState.getAgentState(i) for i in self.getOpponents(gameState)]
    #     enemiesPos = [a.getPosition() for a in enemy if a.getPosition() != None and a.isPacman == False]


    #     reward = 0
    #     if len(enemiesPos) > 0:
    #         currDist = min(self.getMazeDistance(currentAgentState.getPosition(), g) for g in enemiesPos)
    #         nextDist = min(self.getMazeDistance(nextAgentState.getPosition(), g) for g in enemiesPos)
    #     else:
    #         currDist = nextDist = 0

    #     print("current distance------:",currDist, "::::nextDist distance ------:",nextDist)
    ...
    def getOffensiveFeatures(self, gameState: GameState, action):
        ...
        # features["#-of-ghosts-2-steps-away"] = sum(
        #     any((next_x, next_y) in Actions.getLegalNeighbors(n, walls) for n in Actions.getLegalNeighbors(g, walls))
        #     for g in ghosts
        # )
        ...
        # gameState.getAgentPosition(self.getOpponents(gameState)[0])) / (walls.width * walls.height) if  gameState.getAgentState(self.getOpponents(gameState)[0]).ispacman == False else 0

        # if there's no enemy and eat food
    ...
    def getDefensiveFeatures(self, gameState, action):
        ...
        # features['teamDistance'] = team_dist / float(walls.width * walls.height)
        ...
        # if we face confloc
        tie = [a for a in enemies if not a.isPacman and a.getPosition() != None]
        dist_tie = [self.getMazeDistance(prePos, a.getPosition()) for a in tie]
        if len(tie) > 0:
            dist_tie = min(dist_tie)
            if dist_tie == 1 or dist_tie == 2:
        ...
        if len(invaders) > 0:  # invaders present
            ...
            # min_invader_distance = min([self.getMazeDistance(successor, a.getPosition()) for a in invaders])
            # features['chaseInvader'] = min_invader_distance/float(walls.width * walls.height)
            features['distance_to_middlle'] = 0
        ...
        # # Add a feature for the number of food pellets left
        # features['numFoodLeft'] = len(food) / (walls.width * walls.height)
    ...
    #     if len(enemiesPos) > 0:
    #         closestEnemyDist = min([self.getMazeDistance(myPos, enemyPos) for enemyPos in enemiesPos]) / (walls.width + walls.height)
